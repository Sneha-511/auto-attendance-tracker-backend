...
const sendResetPasswordEmail = async (to, token) => {
  ...
  Automatic Attendance Tracker<br/>`;
...
const sendVerificationEmail = async (to, token) => {
  ...
  const verificationEmailUrl = `${config.frontendUrl}/auth/verify-email?token=${token}`;
  const text = `Dear user,
...
const sendCredentialsEmail = async (to, password) => {
  const subject = 'Account Created for Automatic Attendace Tracker';

  const loginPageUrl = config.frontendUrl;
